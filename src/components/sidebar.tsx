...
import { LayoutGrid, LogOut, Menu, Search, Settings, X } from "lucide-react";
...
export function Sidebar({ categories, email }: { categories: Category[]; email: string }) {
  ...
  const populatedCategories = useMemo(() => categories.filter((category) => category.services.length > 0), [categories]);
  useEffect(() => {
  ...
  const itemClass = "group relative flex h-10 items-center gap-2.5 rounded-md px-2.5 text-sm leading-none text-muted-foreground transition-[background-color,color] duration-150 hover:bg-hover hover:text-foreground";
  const navigation = <>
    ...
    <Link href="/dashboard/launcher" onClick={() => setOpen(false)} className={cn(itemClass, "mb-5", path === "/dashboard/launcher" && "bg-hover font-medium text-foreground")}><LayoutGrid size={15} />Launcher</Link>
  ...
  return <>
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-60 flex-col border-r border-border bg-sidebar transition-transform duration-[220ms] ease-[var(--ease-drawer)] lg:translate-x-0", !open && "-translate-x-full lg:translate-x-0")}>
      <div className="flex h-16 items-center gap-2 border-b border-border px-3">
        <Link href="/dashboard" onClick={() => setOpen(false)} className="flex min-w-0 flex-1 items-center gap-3 rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <BrandMark small />
          <div className="min-w-0"><p className="truncate text-sm font-semibold leading-tight tracking-[-0.01em]">Dashbored</p><p className="mt-0.5 truncate text-xs leading-none text-muted-foreground">Home Server</p></div>
        </Link>
        <button className="grid h-10 w-10 place-items-center rounded-md transition-[scale,background-color] duration-150 active:scale-[0.96] hover:bg-hover lg:hidden" onClick={() => setOpen(false)} aria-label="Close navigation"><X size={17} /></button>
      </div>
      <div className="p-3"><button onClick={() => setSearchOpen(true)} className="flex h-10 w-full items-center gap-2.5 rounded-md bg-background px-3 text-sm text-muted-foreground shadow-[var(--surface-shadow)] transition-[scale,box-shadow,color] duration-150 ease-out active:scale-[0.96] hover:text-foreground hover:shadow-[var(--surface-shadow-hover)]"><Search size={16} />Search services<span className="mono ml-auto text-xs">⌘K</span></button></div>
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-4">{navigation}</div>
      <div className="border-t border-border p-3">
        <Link href="/dashboard/settings" className={cn(itemClass, path === "/dashboard/settings" && "bg-hover font-medium text-foreground")}><Settings size={15} />Settings</Link>
        <div className="mt-2 flex items-center gap-2 border-t border-border pt-3"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-foreground text-xs font-semibold text-background">{email.slice(0,2).toUpperCase()}</div><span className="min-w-0 flex-1 truncate text-sm text-muted-foreground" title={email}>{email}</span><Button variant="ghost" size="icon" className="h-10 w-10 shrink-0" onClick={async () => { await authClient.signOut(); router.push("/login"); router.refresh(); }} aria-label="Sign out"><LogOut size={15} /></Button></div>
      </div>
    </aside>
    <ServiceSearchDialog open={searchOpen} onOpenChange={setSearchOpen} categories={populatedCategories} />
  </>;
}
