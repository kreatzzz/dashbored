...
function ProductPreview({ slug, kind = "service" }: { slug: string; kind?: Product["preview"] }) {
  ...
  return <div className="relative grid min-h-[300px] place-items-center overflow-hidden border-b border-border bg-muted/20 lg:border-b-0 lg:border-r"><div className="relative grid h-36 w-36 place-items-center rounded-full border border-border"><div className="absolute h-24 w-24 rounded-full border border-border" /><div className="absolute h-12 w-12 rounded-full border border-strong-border" /><Waves size={18} className="text-muted-foreground" /></div><span className="absolute bottom-5 left-6 mono text-[10px] text-muted-foreground">{slug.toUpperCase()} · LAUNCH LINK</span></div>;
}
