...
function LauncherGridTerminus({ span }: { span: number }) {
  return <div aria-hidden="true" className={`relative min-h-[142px] overflow-hidden bg-black ${span === 2 ? "xl:col-span-2" : ""}`}>
    <span className="absolute left-[17%] top-1/2 h-px w-[46%] -translate-y-3 rotate-[-42deg] bg-white/20" />
    <span className="absolute left-[39%] top-1/2 h-px w-[46%] translate-y-3 rotate-[-42deg] bg-white/20" />
  </div>;
}
