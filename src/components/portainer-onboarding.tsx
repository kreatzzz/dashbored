...
export function PortainerConnectionDialog({ replaceUnconfigured = false }: { replaceUnconfigured?: boolean }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm">Connect Portainer <ArrowRight size={13} /></Button></DialogTrigger><DialogContent className="max-w-md">
      ...
      <form action={submit} className="mt-6 space-y-4">{replaceUnconfigured && <input type="hidden" name="replaceUnconfigured" value="true" />}
        ...
        <details className="group rounded-md border border-border bg-muted/20"><summary className="cursor-pointer px-3 py-2.5 text-xs font-medium text-muted-foreground marker:hidden">Advanced connection settings</summary><div className="grid gap-4 border-t border-border px-3 py-4"><Field label="Environment ID (optional)" hint="Leave blank to use the first environment your token can access. Enter an ID only when you manage more than one."><Input name="endpointId" type="number" min="1" step="1" inputMode="numeric" placeholder="Auto-select" /></Field><Field label="Browser URL (optional)" hint="Leave blank to use the Portainer URL above."><Input name="launchUrl" type="url" autoComplete="url" placeholder="https://portainer.home.arpa" /></Field></div></details>
